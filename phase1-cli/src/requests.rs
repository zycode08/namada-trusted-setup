...
async fn submit_request<T>(client: &Client, coordinator_address: &mut Url, endpoint: &str, request_body: Option<&T>, request: &Method) -> Result<Response>
...
pub async fn post_join_queue(client: &Client, coordinator_address: &mut Url, request_body: &String) -> Result<()> {
    submit_request(client, coordinator_address, "contributor/join_queue", Some(request_body), &Method::POST).await?;

    Ok(())
}

/// Send a request to the [Coordinator](`phase1-coordinator::Coordinator`) to lock the next [Chunk](`phase1-coordinator::objects::Chunk`).
pub async fn post_lock_chunk(client: &Client, coordinator_address: &mut Url, request_body: &String) -> Result<LockedLocators> {
    let response = submit_request(client, coordinator_address, "contributor/lock_chunk", Some(request_body), &Method::POST).await?;
...
pub async fn get_chunk(client: &Client, coordinator_address: &mut Url, request_body: &GetChunkRequest) -> Result<Task> {
    let response = submit_request(client, coordinator_address, "download/chunk", Some(request_body), &Method::GET).await?;
...
pub async fn post_chunk(client: &Client, coordinator_address: &mut Url, request_body: &PostChunkRequest) -> Result<()> {
    submit_request(client, coordinator_address, "upload/chunk", Some(request_body), &Method::POST).await?;

    Ok(())
}

/// Send a request to notify the [Coordinator](`phase1-coordinator::Coordinator`) of an uploaded contribution.
pub async fn post_contribute_chunk(client: &Client, coordinator_address: &mut Url, request_body: &ContributeChunkRequest) -> Result<ContributionLocator> {
    let response = submit_request(client, coordinator_address, "contributor/contribute_chunk", Some(request_body), &Method::POST).await?;
...
pub async fn post_heartbeat(client: &Client, coordinator_address: &mut Url, request_body: &String) -> Result<()> {
    submit_request(client, coordinator_address, "contributor/heartbeat", Some(request_body), &Method::POST).await?;

    Ok(())
}

/// Get pending tasks of the contributor.
pub async fn get_tasks_left(client: &Client, coordinator_address: &mut Url, request_body: &String) -> Result<LinkedList<Task>> {
    let response = submit_request(client, coordinator_address, "contributor/get_tasks_left", Some(request_body), &Method::GET).await?;
